...
\frame
{
	...
	\begin{itemize}
		...
		\begin{description}
			\item[5 composites] C-STORE\footnote{Op\'eration}<7->, C-FIND$^1$, C-MOVE$^1$, C-GET$^1$, C-ECHO$^1$.
			\item[6 normalis\'es] N-GET$^1$, N-ACTION$^1$, N-SET$^1$, N-CREATE$^1$, N-DELETE$^1$, N-EVENT-REPORT\footnote{Notification}<8->.
		\end{description}
	\end{itemize}
}
